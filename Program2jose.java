...
public class Program2
{
    public static void main(String[] args)
    {
        ...
        BinaryNumber eq2 = new BinaryNumber("00000101");
        if(eq1.equal(eq2))
            correct += 1;
        else
        ...
        BinaryNumber incBy1 = new BinaryNumber("00000011");
        BinaryNumber incBy2 = new BinaryNumber("00010");
        incBy1.incrementBy(incBy2);
        if(incBy1.toString().equals("00000101"))
            correct += 1;
        else
         ...
         */
        BinaryNumber tooBig = new BinaryNumber("00110101010");
        while(true)
        {
            try
            {
        ...
        BinaryNumber decBy1 = new BinaryNumber("10010");
        BinaryNumber decBy2 = new BinaryNumber("00101");
        decBy1.decrementBy(decBy2);
        System.out.println(decBy1.toString());
        if(decBy1.toString().equals("01101"))
            correct += 1;
        else
        ...
        //A bit of random testing
        
        int points = 1;
        for( int i = 0; i < 100 && points > 0; i++)
        {
            BinaryNumber binNum = new BinaryNumber(randomBin());
            
            BinaryNumber temp = new BinaryNumber();
            
            try
            {
            temp.setValue(binNum.toString());
            
            if( !temp.equal(binNum))
            {
                points--;
                System.out.printf("Test failed on constructor input: %s\n", binNum);
            }
            
            binNum.setValue(randomBin());
            }
            catch(IllegalArgumentException e)
            {}
            ...
            try
            {
                ...
                {
                    points--;
                    System.out.printf("Test failed on setValue input: %s\nOriginal BinaryNumber: %s\n",binNum ,temp.toString());
                }
            }
            catch(IllegalArgumentException e)
            {}
    ...
    //Method used to complete random testing
